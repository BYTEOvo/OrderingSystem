<template>
    ...
    <el-form-item label="菜品描述" prop="description">
        <el-input v-model="dishInfo.description" maxlength="100" ></el-input>
    </el-form-item>
      <el-form-item label="菜品种类" prop="type">
        <el-select v-model="dishInfo.type" placeholder="请选择">
          <el-option
...
<!--    </el-form-item>-->
    <el-form-item>
        <el-button type="primary" @click="submitForm('dishInfo')">修改</el-button>
    ...
    return {
      ...
      rules: {
        ...
        description: [
          ...
          { min: 1, max: 25565, message: '范围1-25565', trigger: 'blur' }
        ]
      },
      // value: '',
      options: []
  ...
  methods: {
    ...
    submitForm (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.put('/dishes/update/' + _this.dishInfo.type, _this.dishInfo).then(function (resp) {
    ...
    getDishTypeList () {
      this.$axios.get('/dishType').then(resp => {
        if (resp && resp.status === 200) {
          ...
          this.dishInfo.type = this.options[this.$route.query.tid - 1].id
        }
      })
    }
  }
